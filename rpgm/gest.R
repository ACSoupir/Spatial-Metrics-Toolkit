...
#calculate gest
calculate_gest = function(config, path){
  #set correct path
  ...
  gest_res = lapply(seq(df_list), function(l){
    ...
    res = lapply(config$variables$markers, function(marker){
      ...
      if(npoints(p2) < 2){
        ...
        out = Gest(p2, r=eval(parse(text = config$variables$radii_range))) %>%
          as.data.frame(check.names = FALSE) %>%
          mutate(!!config$variables$sample_id := id,
                 Marker = marker, 
                 .before = 1)
    ...
    do.call(bind_rows, .)
  fwrite(gest_res,
         file.path(config$paths$output, 'metrics/gest/', paste0(basename(gsub(".csv.*", "", path)), ".csv.gz")),
         compress = "gzip")
