...
#calculate kest
calculate_kcross = function(config, path){
  #set correct path
  ...
  kest_res = lapply(seq(df_list), function(l){
    ...
    res = lapply(config$variables$bivar_pairs, function(markers){
      ...
      if(any(counts < 2)){
        ...
        out = Kcross(p2, r=eval(parse(text = config$variables$radii_range))) %>%
    ...
    dplyr::mutate(!!config$variables$sample_id := unique(df[[config$variables$sample_id]]),
                  .before = 1)
  fwrite(kest_res,
         file.path(config$paths$output, 'metrics/kcross/', paste0(basename(gsub(".csv.*", "", path)), ".csv.gz")),
         compress = "gzip")
...
#plot it
plot_kcross = function(config, path){
  #set correct path
  setwd(config$paths$root)
  ...
  if(!is.null(config$variables$tissue_class_label)){
    ...
    p = df %>%
      ggplot() + 
      geom_line(aes(x = r, y = border - theo, color = Counted)) +
      facet_grid(get(config$variables$tissue_class_label) ~ Anchor) +
      theme_classic()
    
    pdf(file.path(config$paths$output, 'figures/metrics/kcross/', paste0(basename(gsub(".csv.*", "", path)), ".pdf")),
